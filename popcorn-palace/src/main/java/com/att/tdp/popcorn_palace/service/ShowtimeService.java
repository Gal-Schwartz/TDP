...
public class ShowtimeService {
    ...
    public Showtime addShowtime(Showtime s) {
        List<Showtime> overlaps = showtimeRepository.findByTheater(s.getTheater())
                .stream()
                .filter(show -> !(show.getEndTime().isBefore(s.getStartTime()) || show.getStartTime().isAfter(s.getEndTime())))
                .toList();
